...
export const BcryptHashingForgotPassX = (codexID) => {
    let codexIdAlphaNumStr = bcrypt.hashSync(codexID, 10);
    return codexIdAlphaNumStr;
};
export const BcryptHashingForgotPassY = (codeyID) => {
    let codeyIdAlphaNumStr = bcrypt.hashSync(codeyID, 10);
    return codeyIdAlphaNumStr;
};
export const BcryptCompareResetPassX = async (codexid, codex) => {
    const matchX = await bcrypt.compare(codexid, codex);
    return matchX;
};
export const BcryptCompareResetPassY = async (codeyid, codey) => {
